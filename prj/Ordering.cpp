...
namespace seneca {
    ...
    void Ordering::loadDrinks(const char* drinkFile) {
        ...
        if (file) {
            while (!file.eof()) {
                Drink* drink = new Drink(); // Tạo đối tượng Drink rỗng
                drink->read(file); // Sử dụng hàm read() để đọc thông tin vào đối tượng Drink
                drinkMenu.addOption(drink); // Thêm đồ uống vào menu
            }
        }
    }
    ...
    void Ordering::loadFoods(const char* foodFile) {
        ...
        if (file) {
            ...
            double price;
            while (!file.eof()) {
                Food* food = new Food(); // Tạo đối tượng Food rỗng
                food->read(file); // Sử dụng hàm read() để đọc thông tin vào đối tượng Food
        ...
        if (selection > 0 && selection <= foodMenu.size()) {
            ...
            if (selectedFood != nullptr) {
                if (selectedFood->order()) {
                    ...
                    std::cout << "Đặt món thành công.\n";
                } else {
                    delete selectedFood;
                    std::cout << "Đặt món thất bại.\n";
                }
            }
        } else if (selection == 0) {
            std::cout << "Quay lại menu chính.\n";
        }
    }
    ...
    void Ordering::printBill(std::ostream& os) const {
        ...
        for (int i = 0; i < numBillItems; ++i) {
            ...
            if (item) {
                // Giả sử Billable có phương thức print() để in chi tiết từng món
                item->print(os);  
                totalPrice += item->price();  // Thêm giá vào tổng
            }
        }

        // Giả sử có phương thức printTotals để in tổng giá trị hóa đơn
        printTotals(os, totalPrice);
    }

    void Ordering::resetBill() {
        // Tạo bộ nhớ đệm để lưu tên file
        char filename[21];
        Utils utils;
        ...
        // Xóa các phần tử trong `billItems`
        ...
        // Cập nhật trạng thái
        ++billNumber; // Chuyển sang hóa đơn tiếp theo
    }
